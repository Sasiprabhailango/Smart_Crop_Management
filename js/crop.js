...
const harvesting = document.getElementById("harvesting");
    ...
    cropSelect.addEventListener("change",() =>{
        ...
        if(crop){
          result.style.display = "block"; 
            result.style.backgroundImage =`url("images/${cropSelect.value.toLowerCase()}.jpeg")`;

            console.log(`images/${cropSelect.value.toLowerCase()}.jpeg`);

            water.textContent = crop.Water;
            fertilizer.textContent = crop.Fertilizer;
            soil.textContent = crop.Soil;
            disease.textContent = crop.Disease;
            harvesting.textContent = crop.Harvesting;

            cropImage.src = `images/${cropSelect.value.toLowerCase()}.jpeg`;
            cropImage.alt = cropSelect.value;
        }
    });
}
